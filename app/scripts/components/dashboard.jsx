...
var Dashboard = React.createClass({
  ...
  buildChart: function() {
    ...
    nv.addGraph(function() {
      ...
      .options({
        margin: {left: 75, bottom: 40, right: 30},
      ...
      .tickFormat(function(d, i) {
        return (d3.time.format('%e %b %y')(moment().subtract('days', 14).add('days', d).toDate()));
      });
      // .tickFormat(d3.format(',.1f'));

      chart.yAxis
      .axisLabel('Email Opens')
...
function getData() {
  ...
  for (var i = 0; i < 15; i++) {
    rand.push({key: "a", x:i, y: Math.floor(Math.random()*400) + 100});
  }
